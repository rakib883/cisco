...
useEffect(()=>{
  const incomingData = async()=>{
      try{
        const response = await FachingData("http://localhost:3000/trending-product")
...
// trending data catagory filter area start
 const [catagoryData,setCatagoryData] = useState("")
    const catagoryHandeler = async()=>{
        if(catagoryData){
            try{
              const response = await fetch(`http://localhost:3000/${catagoryData.link}`)
  ...
  const [color,setColor] = useState("")
    const colorFilter = async()=>{
      try{
          const response = await FachingData(`http://localhost:3000/color/filter/${color}`)
  ...
  const sizeFilterHandeler =async()=>{
      try{
         const response = await FachingData(`http://localhost:3000/product/color/${size}`)
         if(response.length > 0){
          setProductData(response)
         }
         setInnercontent(0)
  ...
  return (
    <div>
        <div className="content mx-10 mt-[50px]">
            ...
            <div className="menu">
              <div className="item my-4 flex justify-between items-center">
                  <div className="menu flex gap-4">
                    <div onClick={()=>setHover(1)} className="menu-item cursor-pointer">
                       <p className={`${hoverStyle === 1 ? "bg-black text-white" : "text-[#66748b] hover:bg-slate-100  " }  font-semibold px-6 py-2 rounded-full`}>All Items</p>
                    </div>
                  ...
                  <div className="button">
                    <button className=" flex items-center gap-2 bg-black text-white px-6 py-2 rounded-full">
                      <LuFilter className=" text-xl" /> <p className="S font-sans font-semibold ">Filter</p> <MdOutlineKeyboardArrowUp className="ml-4 text-2xl mt-[3px]" />
                    </button>
                  </div>
              </div>
            </div>
            ...
            {/* product area strat */}
            <div className="maon my-6  grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {
