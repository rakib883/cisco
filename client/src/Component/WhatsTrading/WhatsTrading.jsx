...
useEffect(()=>{
  const incomingData = async()=>{
      try{
        const response = await FachingData("https://cisco-server.vercel.app/api/product/trending-product")
...
// trending data catagory filter area start
 const [catagoryData,setCatagoryData] = useState("")
    const catagoryHandeler = async()=>{
        if(catagoryData){
            try{
              const response = await fetch(`https://cisco-server.vercel.app/api/product/filter/${catagoryData.link}`)
  ...
  const [color,setColor] = useState("")
    const colorFilter = async()=>{
      try{
          const response = await FachingData(`https://cisco-server.vercel.app/api/product/filter/color/${color}`)
  ...
  const sizeFilterHandeler =async()=>{
      try{
         const response = await FachingData(`https://cisco-server.vercel.app/api/product/filter/size/${size}`)
         if(response.length > 0){
          setProductData(response)
         }
         setInnercontent(0)
